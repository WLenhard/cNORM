...
knitr::opts_chunk$set(tidy.opts=list(width.cutoff=60),
  collapse = TRUE,
  comment = "#>",
  eval = FALSE  # Disabled due to computational intensity
)
```
...
The ShaSh distribution is based on a two-step transformation of the standard normal distribution, allowing independent control over distributional shape and location-scale characteristics. This two-step approach separates shape control (Step 1) from location-scale adjustment (Step 2), enabling flexible modeling while preserving mathematical properties. If Y follows a standard normal distribution N(0,1), then the ShaSh-transformed variable Z is defined as (Step 1):

$$Z = \sinh(\delta \cdot \text{arcsinh}(Y) + \epsilon)$$

This transformation controls the distributional shape through the parameters ε (skewness) and δ (tail weight). The final ShaSh-distributed variable X is obtained by applying location and scale parameters (Step 2):
...
where Φ is the standard normal CDF and z = (x - μ)/σ. The quantile function (inverse CDF) can be expressed as:
...
The tail weight parameter δ is held constant across ages in the current implementation, though it can be adjusted to reflect population characteristics. Age is standardized as: $\text{age}_{std} = \frac{\text{age} - \overline{\text{age}}}{\text{SD}(\text{age})}$ for numerical stability during optimization.
...
model.custom <- cnorm.shash(age = ppvt$age, score = ppvt$raw,
                           mu_degree = 4,        # More flexible location pattern
...
```{r fig2alt, fig.height = 4, fig.width = 7}
