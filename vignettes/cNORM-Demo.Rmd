...
The 'cnorm' method combines most of the steps in one go. The example in a nutshell already suffices for establishing norm scores. A detailed explanation follows afterwards. 
...
# The method 'cnorm' prepares the data and establishes a model.
...
plot.series(model, end = 12)   # series of percentile plots
plot.subset(model)             # plot information function
cv(model$data)                 # run repeated cross validation
...
table(c(3, 3.2, 3.4, 3.6), model, CI = .9, reliability = .94)
...
Please note that the 'rankBySlidingWindow' function only makes sense if the age variable is actually continuous. In the 'elfe' data set the variable 'group' serves as continuous explanatory variable as well as discrete grouping variable. Therefore, with the function 'rankBySlidingWindow' we obtain the same standard scores as with the function 'rankByGroup' in this specific case. In order to compensate for imbalances in the representativeness of the dataset, weights can be applied via the 'weights' parameter. In that case, weighted percentiles are computed and the weights are as well used in the regression modeling.
...
cv(cnorm.elfe$data, max=10, repetitions = 2)
```
...
plot.series(cnorm.elfe, terms = 10)
```
...
plot.derivative(cnorm.elfe, minAge=1, maxAge=6, minNorm=20, maxNorm=80)
...
```
getNormCurve(50, cnorm.elfe, minAge = 2, maxAge = 5, step = 0.25, minRaw = 0, maxRaw = 28)
```
...
plot.curves(cnorm.elfe, normList = c(30, 40, 50, 60, 70), minAge = 2, maxAge = 5, step = 0.1, minRaw = 0, maxRaw = 28)
```
...
```{r}
predict.norm(15, 4.7, cnorm.elfe, minNorm = 25, maxNorm = 75)
```
...
table(3, cnorm.elfe, minRaw = 0, maxRaw = 28, minNorm=30, maxNorm=70, step = 1)

# You as well can generate multiple tables at once, by providing a vector for the age groups,
# here for grade 3, 3.5, 4 and 4.5
normTables -> normTable(c(3.5, 4.5), cnorm.elfe, minRaw = 0, maxRaw = 28, 
