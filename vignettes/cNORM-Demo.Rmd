...
plot.series(model, start=4, end = 8)   # series of percentile plots
...
plotPercentileSeries(model, terms = 10)
```
...
```{r fig4b, fig.height = 4, fig.width = 7}
plotNorm(model)
```
The plot can be split by group as well:
```{r fig4c, fig.height = 4, fig.width = 7}
