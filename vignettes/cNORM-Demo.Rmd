...
# do a cross validation with 5 repetition, restrict number of max terms to 10
cnorm.cv(d, max=10, repetitions = 5)
```
<img src="cnorm.cv.png" align=center style="border:0;">

In this specific case, it might not be wortwhile to use more terms than 4, because $R^{2}$ in the cross validated data set does not increase anymore.You should however as well inspect to percentile curves visually or use the 'checkConsistency' function on the final model to avoid intersecting percentile curves and use a model with more terms in that case.
