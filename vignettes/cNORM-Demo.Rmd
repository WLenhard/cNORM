...
So far, all examples in this vignette have modelled norms directly from **raw scores**. cNORM is, however, agnostic to the metric of the score variable — any quantitative person parameter can serve as input. A particularly attractive option is to combine continuous norming with **Item Response Theory (IRT)**: instead of summing item responses, one estimates a latent ability $\theta$ for every person via an IRT model and then norms these $\theta$ values against age (or any other explanatory variable). This has two advantages: ability estimates are placed on an interval-level metric, and item-level information (difficulty, discrimination, missing responses) is taken into account by the measurement model.
...
The call to `cnorm()` is identical to the one used for raw scores — only the input variable changes. All subsequent functions (`predictNorm()`, `plot()`, `getNormCurve()`, …) work exactly as before, but now operate on IRT-scaled ability estimates. This makes the Taylor polynomial approach a flexible post-processing step for virtually any psychometric scoring procedure, including 2PL, 3PL, partial credit or rating-scale models.
