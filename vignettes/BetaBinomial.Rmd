...
Let's walk through a practical example using the ppvt dataset, included in cNORM. Before we begin, ensure you have the following packages installed and loaded:
...
# In order to use different optimization functions, you can specify different control parameters,
# e.g., including methods from the optimx package. The default is the BFGS algorithm.
```
...
`cnorm.betabinomial` already displays a plot of the raw scores and the percentile curves. This can also be done manually using the `plot.betabinomial` function. Observe if the percentile trajectories exhibit plausible forms. To predict $\mu$ and $\sigma$ for specific ages or times, as well as the $\alpha$ and $\beta$ parameters, you can use the `predict.cnormBetaBinomial` method:
